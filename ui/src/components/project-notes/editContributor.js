...
export default function EditContributor() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetch(`http://localhost:5000/project_notes/contributor/${params.id.toString()}`);
  ...
  async function onSubmit(e) {
    ...
    await fetch(`http://localhost:5000/project_notes/contributor/update/${params.id}`, {
      method: "POST",
