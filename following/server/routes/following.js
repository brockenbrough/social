...
followerRoutes.route("/following/follower").get(function (req, res) {
...
followerRoutes.route("/following/follower/:id").get(function (req, res) {
...
followerRoutes.route("/following/follower/add").post(function (req, response) {
  let db_connect = dbo.getDb();
  let myobj = {
    name: req.body.name,
...
followerRoutes.route("/following/follower/update/:id").post(function (req, response) {
  let db_connect = dbo.getDb();
  let myquery = { _id: ObjectId( req.params.id )};
  let newvalues = {
    $set: {
...
followerRoutes.route("/following/follower/:id").delete((req, response) => {
  let db_connect = dbo.getDb();
  let myquery = { _id: ObjectId( req.params.id )};
