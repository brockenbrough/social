...
followerRoutes.route("/followers/follow").post(function (req, response) {
  if (req.body.userId == null || req.body.userId == "")
    return response
      .status(400)
      .send({ message: "Invalid parameters for userId." });
  if (req.body.targetUserId == null || req.body.targetUserId == "")
    return response
      .status(400)
      .send({ message: "Invalid parameters for targetUserId." });
      ...
      if (res) {
        ...
        db_connect
          ...
          .insertOne(createFollowing, function (err, res) {
            if (err) throw err;
            console.log(
              "Created new User " + req.body.userId + ",and added to following."
            );
            response.json(res);
          });
      }
...
followerRoutes.route("/followers/deleteFollower").delete((req, response) => {
  ...
  if (req.body.targetUserId == null || req.body.targetUserId == "")
    return response
      .status(400)
      .json("Invalid parameters to DELETE targetUserId from follower list.");
  ...
  db_connect
    ...
    .updateOne(
      { userId: req.body.userId },
      { $pull: { followers: req.body.targetUserId } },
      function (err, obj) {
        if (err) throw err;
  ...
  db_connect
    ...
    .updateOne(
      ...
      function (err, obj) {
        if (err) throw err;
        console.log(
          "A following(s) has been deleted from " + req.body.targetUserId
        );
      }
    );
});
...
followerRoutes.route("/followers/unfollowUser").delete((req, response) => {
  ...
  if (req.body.targetUserId == null || req.body.targetUserId == "")
    return response
      .status(400)
      .json("Invalid parameters to UNFOLLOW targetUserId from following list.");
